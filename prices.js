...
function openUniversalQualitySheet(serviceName) {
    ...
    listEl.innerHTML = `
        ...
        <li><button class="custom-sheet-item" onclick="if(window.Telegram?.WebApp?.HapticFeedback)Telegram.WebApp.HapticFeedback.impactOccurred('light'); selectQualityFromSheet('dev', '💎 Pro (2k)')"><span>Pro (1k)</span><span class="coin-price">${mid} 🪙</span></button></li>
        <li><button class="custom-sheet-item" onclick="if(window.Telegram?.WebApp?.HapticFeedback)Telegram.WebApp.HapticFeedback.impactOccurred('light'); selectQualityFromSheet('ultra_4k', '🔥 Ultra (4k)')"><span>Ultra (1k)</span><span class="coin-price">${max} 🪙</span></button></li>
    `;
